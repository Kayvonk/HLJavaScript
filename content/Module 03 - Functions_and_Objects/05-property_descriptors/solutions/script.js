...
// Property descriptors expose the metadata behind every own property. Regular
// assignment defaults writable/enumerable/configurable to TRUE, but
// defineProperty defaults them all to FALSE unless explicitly set.
// enumerable: false hides the property from Object.keys, for...in, and JSON.stringify.
...
console.log("Object.keys(settings):", Object.keys(settings)); // ["theme"]
console.log("settings.version:", settings.version);           // "1.0.0"
...
// Write your code here:
// Object.freeze makes an object immutable at the top level: it sets every
// property's writable AND configurable to false and prevents extensions.
// It's a shallow freeze — nested objects are not frozen unless done recursively.
const frozen = { name: "Widget", price: 10 };
Object.freeze(frozen);

frozen.color = "red";
// sloppy mode: silently ignored. strict mode: throws TypeError.
console.log("after add attempt:", frozen);

frozen.price = 99;
// sloppy mode: silently ignored. strict mode: throws TypeError.
console.log("after modify attempt:", frozen);

delete frozen.name;
// sloppy mode: silently ignored. strict mode: throws TypeError.
console.log("after delete attempt:", frozen);
