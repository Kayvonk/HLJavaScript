...
// pipe reads left-to-right — matches reading/data-flow order. reduce feeds
// each function's output into the next; because every step is pure, the
// whole pipeline is referentially transparent and trivially unit-testable.
// (Note: the example above already defines a `pipe`; we shadow it in a
// local block-scoped const to keep the TODO self-contained.)
console.log("\n--- TODO 1: pipe ---");
{
  const pipe = (...fns) => x => fns.reduce((v, f) => f(v), x);
  ...
  const prefix   = s => "slug-" + s;

  const toSlug = pipe(trim, lower, dashify, prefix);
...
// Write your code here:
// compose follows the mathematical convention f∘g∘h where h runs first.
// reduceRight walks the array from the last function back to the first,
// so writing `compose(a, b, c)` is equivalent to `x => a(b(c(x)))`.
console.log("\n--- TODO 2: compose ---");
{
  ...
  const viaCompose = compose(prefix, dashify, lower, trim)(input);
  const viaPipe    = pipe(trim, lower, dashify, prefix)(input);

  console.log("compose:", viaCompose);           // "slug-hello-world"
  console.log("pipe:   ", viaPipe);              // "slug-hello-world"
  console.log("equal?  ", viaCompose === viaPipe); // true
}
